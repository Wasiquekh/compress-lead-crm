        ...
        <div className=" hidden md:w-auto md:flex md:justify-end md:items-center md:gap-7 w-auto z-10">
          <div className="flex gap-2">
            <input
              ...
              className=" w-64 h-[50px] rounded-[4px] px-6 border border-primary-400 hover:shadow-hoverInputShadow focus:border-primary placeholder-gray-400 text-secondBlack text-[15px] leading-normal outline-none"
              value={query}
            ...
            {notificationData.length > 0 ? (
              notificationData.map((notification, index) => (
                <li
                  key={index}
                  onClick={() => navigateLeadDetails(notification.lead_id)}
                  className="px-3 py-3 text-sm hover:cursor-pointer hover:bg-primary-100 odd:bg-primary-50 text-black"
                >
                  {notification.body}
                </li>
              ))
