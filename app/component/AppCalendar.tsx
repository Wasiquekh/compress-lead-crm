...
export default function AppCalendar({
  ...
  return (
 <div className="relative overflow-x-auto w-full sm:rounded-lg  text-secondBlack">
  <table className="w-full text-sm text-left whitespace-nowrap">
    <thead className="text-xs  text-secondBlack">
      <tr className="  ">
        <th className="px-3 py-3 md:p-3    font-semibold  text-base">
          <div className="flex items-center gap-2">
            <RxAvatar className="w-5 h-5 sm:w-6 sm:h-6" />
            <span>Name - Mail</span>
          </div>
        </th>
        <th className="px-3 py-2    hidden md:table-cell font-semibold  text-base">
          <div className="flex items-center gap-2">
      ...
      {!tasks || tasks.length === 0 ? (
        ...
        tasks.map((t, index) => {
          ...
          return (
            <tr
              key={t?.id ?? index}
              className="    hover:bg-primary-100 transition-colors border-b border-[#E7E7E7] odd:bg-primary-50 text-secondBlack"
            >
              {/* Start Date */}
              <td className="px-1 py-2 md:px-3 md:py-2 border-tableBorder">
                <p className=" text-sm sm:text-base">{t?.start_at || "-"}</p>
              </td>

              {/* Subject */}
              <td className="px-3 py-2    hidden md:table-cell">
                <p className=" text-sm sm:text-base capitalize">{t?.subject || "-"}</p>
