...
const INITIAL_VALUES = {
  ...
  const fetchLeadDocumentData = async () => {
    ...
    try {
      ...
      console.log("lead document data22222222222", res.data.data);
      setDocs(res.data.data); // <-- if you want to store in state
    } catch (error: any) {
      console.error("Error fetching lead:", error);
    }
  };
...
const provinceOptions = [
            ...
            {fetchLeadActivityData && fetchLeadActivityData.length > 0 ? (
              fetchLeadActivityData.map((activity) => {
                ...
                return (
                  <div
                    key={activity.id}
                    className="w-full flex justify-between gap-4 hover:bg-primary-100 py-2 px-2 rounded transition-colors border-b border-[#E7E7E7] odd:bg-primary-50 text-secondBlack"
                  >
  ...
  {(docs?.length ?? 0) === 0 ? (
    ...
    (docs ?? []).map((d) => (
      <div
        key={d.id}
        className="grid grid-cols-[30%_1fr] gap-4 p-3 border border-white rounded hover:bg-primary-100  odd:bg-primary-50 transition-colors text-secondBlack"
      >
        {/* Left Column: Notes */}
        <div className="flex items-center">
          <p className="text-base  whitespace-pre-wrap capitalize">
            {d.notes || "—"}
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2 md:gap-3 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap mt-2 md:mt-0">
            <a
              href={d.download}
              download
              className="py-2 px-3 border border-gray-600 rounded text-sm  hover:bg-primary-500 hover:text-white transition-colors"
            >
              Download
            </a>
            ...
            <button
              ...
              className="py-2 px-3 border border-gray-600 rounded text-sm  hover:bg-primary-500 hover:text-primary-700 transition-colors"
            >
              <MdEdit />
            </button>
            ...
            <button
              ...
              className="py-2 px-3 border border-red-500 rounded text-sm  hover:bg-red-600 hover:text-primary-700 transition-colors"
            >
              <RiDeleteBin6Line />
            </button>
          </div>
  ...
  // --------------------------- DATE HELPER
// Time slot config (no timezone needed)
const SLOT_MINUTES = 15;
const DURATION_MINUTES = 30;

// Simple local time functions
const toPickerLocal = (d: Date | null) => d;
const fromPickerLocal = (d: Date | null) => d;
const nowLocal = () => new Date();

// Round UP to next SLOT_MINUTES
const roundUpToSlot = (d: Date = nowLocal()) => {
...
// Start of today for minDate
const startOfTodayLocal = () => {
  const n = nowLocal();
  return new Date(n.getFullYear(), n.getMonth(), n.getDate());
};

// Next valid START time (rounded to next slot)
const nextValidStartLocal = (base: Date = nowLocal()) => {
  const rounded = roundUpToSlot(base);
  // If rounded time is in past (just in case), add one more slot
  if (rounded <= base) {
...
// Formatter for payload (local time)
...
// ---------- Defaults ----------
const defaultStart = nextValidStartLocal();
const defaultEnd = addMinutes(defaultStart, DURATION_MINUTES);
                        ...
                        <table className="w-full text-sm text-left text-white">
                          <thead className="text-xs ">
                            <tr className="border ">
                              <th
                                scope="col"
                                colSpan={2}
                                className="px-3 py-3 md:p-3 border border-gray-700 font-semibold text-secondBlack text-base"
                              >
                          ...
                          <tbody>
                            {[
                              ...
                              <tr
                                key={idx}
                                className="border    transition-colors border-b border-[#E7E7E7] odd:bg-primary-50 hover:bg-primary-100 text-secondBlack"
                              >
  ...
  <table className="w-full text-sm text-left text-white">
    ...
    <tbody>
      {/* Agent -> CustomSelect */}
      <tr className="border border-tableBorder hover:bg-primary-100 transition-colors text-secondBlack">
        <td className="text-sm py-4 px-4">
          Agent Name
        </td>
      ...
      <tr className="border border-tableBorder hover:bg-primary-100 transition-colors">
        <td className="text-sm py-4 px-4 text-secondBlack">
          Best time to call
        </td>
        <td className="py-4 px-4">
          <Field
            ...
            type="text"
            className="w-full border border-primary-500 rounded-[4px] text-secondBlack bg-white text-sm px-3 py-2 focus:outline-none"
      ...
      {/* Debt Consolidation Status -> CustomSelect */}
      <tr className="border border-tableBorder hover:bg-primary-100 transition-colors">
        <td className="text-sm text-secondBlack py-4 px-4">
          Debt Consolidation Status
        </td>
        <td className="py-4 px-4">
          <CustomSelect
            value={debtConsValue?.id || ""}
            onChange={(value) => {
              setFieldValue("debt_consolidation_status_id", value);
            }}
      ...
      {/* Consolidation Status -> CustomSelect */}
      <tr className="border border-tableBorder hover:bg-primary-100 transition-colors">
        <td className="text-sm text-secondBlack py-4 px-4">
        ...
        <td className="py-4 px-4">
          <CustomSelect
            value={consValue?.id || ""}
            onChange={(value) => {
              setFieldValue("consolidated_credit_status_id", value);
            }}
      ...
      {isSubmitting ? "Saving..." : "Save"}
                  ...
                  <div className=" md:flex relative w-full">
                    ...
                    <GrPowerReset
                      ...
                      className=" absolute -top-5 -right-1 md:top-2 md:right-1 cursor-pointer text-lg md:text-2xl text-secondBlack hover:text-primary-500 active:text-primary-600"
                    />
                  </div>
                </div>
                {userRole === "Agent" && (
  ...
  <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:justify-between mb-4 sm:mb-6">
    ...
    <div className="w-full relative">
      ...
      <CustomSelect
        ...
        onChange={(value) => {
          const selectedOption = (disposition || []).find(
            (opt: any) => opt.id === value
          );
          const name = selectedOption ? selectedOption.name : "";
          setFieldValue("disposition_id", value);
          setFieldValue(
            "conversation",
        ...
        options={(disposition || []).map((opt: any) => ({
          value: opt.id,
          label: opt.name,
          ...opt
        }))}
    ...
    <div className="w-full relative">
      ...
      {loggedInUserRole === "Agent" ? (
        ...
        <div className="w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium py-3 px-4 bg-white text-secondBlack opacity-70">
    ...
    <div className="w-full relative mt-4 col-span-2">
      ...
      <textarea
        ...
        className="hover:shadow-hoverInputShadow focus-border-primary w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4 bg-white text-secondBlack"
        rows={5}
      />
...
>
  {({
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:justify-between mb-4 sm:mb-6">
        ...
        <div className="w-full relative">
          ...
          <input
            ...
            readOnly
            className="capitalize w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4 bg-white text-secondBlack cursor-not-allowed"
          />
        </div>
        ...
        <div className="w-full relative">
          ...
          <input
            ...
            className="w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4 bg-white text-secondBlack"
          />
        ...
        {/* Schedule date: From date helper*/}
        <div className="w-full md:col-span-2">
          <p className="text-secondBlack font-medium text-base leading-6 mb-3">
            Schedule
          </p>

          {/* From */}
          <div className="w-full relative mb-4">
            <p className="text-secondBlack font-medium text-sm leading-6 mb-2">
              From
            </p>
            <DatePicker
              ...
              onChange={(date: Date | null) => {
                ...
                if (date)
                  setFieldValue(
                    "end_at",
                    addMinutes(date, DURATION_MINUTES)
                  );
              }}
              ...
              filterTime={(candidate) => {
                const now = nowLocal();
                // If selecting today's date, only allow future times
                if (
                  candidate.getDate() === now.getDate() &&
                  candidate.getMonth() === now.getMonth() &&
                  candidate.getFullYear() === now.getFullYear()
                ) {
                  return candidate >= nextValidStartLocal();
                }
                return true; // All times allowed for future dates
              }}
              dayClassName={(date) => {
                const today = nowLocal().toDateString();
                const selected = values.start_at
                  ? values.start_at.toDateString()
                  : null;
                if (today === date.toDateString())
                  return "bg-[#FFF0F1] text-[#A3000E]";
                if (selected === date.toDateString())
          ...
          <div className="w-full relative">
            ...
            <DatePicker
              ...
              placeholderText="MM-dd-yyyy hh:mmam/pm"
              disabled
              className="w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4 bg-white text-secondBlack cursor-not-allowed"
              popperClassName="custom-datepicker"
        ...
        <div className="w-full relative md:col-span-2">
          ...
          <textarea
            ...
            rows={4}
            className="w-full border border-gray-700 rounded-[4px] text-sm leading-5 font-medium placeholder-gray-400 py-4 px-4 bg-white text-secondBlack resize-y"
          />
        </div>
      </div>
        ...
        {document && (
          <div className="w-full min-h-auto  text-white p-4">
            ...
            <form onSubmit={handleSubmitDocument} encType="multipart/form-data">
              <div className="w-full">
                <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:justify-between mb-4 sm:mb-6 text-secondBlack">
                  {/* Document Name */}
                  <div className="w-full">
                    ...
                    <input
                      ...
                      required
                      className="hover:shadow-hoverInputShadow focus-border-primary w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4 text-secondBlack bg-white"
                    />
                  </div>
    ...
    .csv
  "
                      className="hover:shadow-hoverInputShadow focus-border-primary w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4  text-secondBlack"
                    />
                  </div>
                </div>
          ...
          <div className="w-full min-h-auto  text-white p-4">
            ...
            >
              {(formik) => {
                ...
                <form onSubmit={handleSubmit} noValidate>
  {/* Date Range */}
  <div className="w-full flex flex-col md:flex-row gap-4 md:justify-between mb-4 sm:mb-6 text-secondBlack">
    <div className="w-full md:w-[49%]">
      ...
      <DatePicker
        ...
        className="hover:shadow-hoverInputShadow focus-border-primary !w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4 text-secondBlack shadow-sm"
        popperClassName="custom-datepicker"
      />
    </div>
  ...
  <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:justify-between mb-4 sm:mb-6 text-secondBlack">
    <div className="w-full relative">
      ...
      <input
        ...
        onBlur={() => setFieldTouched("conversation", true)}
        placeholder="Enter conversation"
        className="hover:shadow-hoverInputShadow focus-border-primary w-full border border-gray-700 rounded-[4px] text-sm leading-4 font-medium placeholder-gray-400 py-4 px-4  text-se
        condBlack bg-white"
      />
    </div>
    ...
    <div className="w-full relative">
      ...
      <CustomSelect
        value={values.disposition_id}
        onChange={(value: any) => setFieldValue("disposition_id", value)}
        onBlur={() => setFieldTouched("disposition_id", true)}
        options={disposition.map((opt: Option) => ({
          value: opt.id,
          label: opt.name
        }))}
        placeholder="Select Disposition"
    ...
    <div className="w-full relative">
      ...
      <CustomSelect
        value={values.agent_id}
        onChange={(value: any) => setFieldValue("agent_id", value)}
        onBlur={() => setFieldTouched("agent_id", true)}
        options={agent.map((opt: Option) => ({
          value: opt.id,
          label: opt.name
        }))}
        placeholder="Select Agent"
  ...
  initialValues={{
    ...
    end_at:
      (taskEditObject?.end_at
        ...
        : taskEditObject?.start_at
        ? addMinutes(new Date(taskEditObject.start_at), DURATION_MINUTES)
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:justify-between mb-4 sm:mb-6">
          ...
          <div className="w-full md:col-span-2">
            ...
            <div className="w-full relative mb-4">
              <div className="w-full relative mb-4">
                ...
                <DatePicker
                  ...
                  dateFormat="MM-dd-yyyy h:mma"
                  placeholderText="MM-dd-yyyy hh:mmam/pm"
                  className="hover:shadow-hoverInputShadow focus-border-primary !w-full border border-[#DFEAF2] rounded-[4px] text-sm leading-4 font-medium placeholder-[#717171] py-4 px-4  shadow-s"
                  popperClassName="custom-datepicker"
                  minDate={startOfTodayLocal()}
                  filterTime={(candidate: Date) => {
                    const now = nowLocal();
                    
                    // If editing an existing task, allow any time (including past)
                    if (isEditing && taskEditObject?.start_at) {
                      const originalDate = new Date(taskEditObject.start_at);
                      // If editing same day as original, allow all times
                      if (
                        candidate.getDate() === originalDate.getDate() &&
                        candidate.getMonth() === originalDate.getMonth() &&
                        candidate.getFullYear() === originalDate.getFullYear()
                      ) {
                        return true;
                      }
                    }
                    
                    // For today's date, only allow future times
                    if (
                      candidate.getDate() === now.getDate() &&
                      candidate.getMonth() === now.getMonth() &&
                      candidate.getFullYear() === now.getFullYear()
            ...
            <div className="w-full relative">
              ...
              <DatePicker
                ...
                className="hover:shadow-hoverInputShadow focus-border-primary 
                  !w-full border border-[#DFEAF2] rounded-[4px] text-sm leading-4 
                  font-medium placeholder-[#717171] py-4 px-4  cursor-not-allowed"
                popperClassName="custom-datepicker"
                dayClassName={() => "pointer-events-none"}
              />
          ...
          <div className="w-full relative md:col-span-2">
            ...
            <textarea
              ...
              placeholder="Add description (optional)"
              rows={4}
              className="hover:shadow-hoverInputShadow focus-border-primary w-full border border-[#DFEAF2] rounded-[4px] text-sm leading-5 font-medium placeholder-[#717171] py-4 px-4  resize-y"
            />
          </div>
        </div>
