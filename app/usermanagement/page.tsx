    ...
    <>
      <div className=" flex justify-end  min-h-screen">
        ...
        <div className="ml-[97px] w-full md:w-[90%] m-auto  min-h-[500px]  rounded p-4 mt-0 ">
          ...
          <div className="rounded-3xl   py-6 px-1  md:p-6 z-10 relative shadow-lastTransaction">
      ...
      <tr className="   ">
        ...
        <th className="px-2 py-1     hidden md:table-cell">
          <div className="flex items-center gap-2">
            ...
            <div className="font-semibold  text-base leading-normal">Email</div>
          </div>
        </th>
        <th className="px-2 py-1     hidden md:table-cell">
          <div className="flex items-center gap-2">
            <MdOutlineCall className="w-5 h-5" />
            <div className="font-semibold  text-base leading-normal">Phone</div>
          </div>
        </th>
        <th className="px-2 py-1     hidden md:table-cell">
          <div className="flex items-center gap-2">
            <LiaArrowCircleDownSolid className="w-5 h-5" />
            <div className="font-semibold  text-base leading-normal">Role</div>
          </div>
        </th>
        <th className="px-2 py-1    ">
          <div className="flex items-center gap-2">
            <LiaArrowCircleDownSolid className="w-5 h-5" />
            <div className="font-semibold  text-base leading-normal">Action</div>
          </div>
        </th>
      </tr>
    </thead>
        ...
        data.map((item: any, index: number) => (
          ...
          >
            ...
            <td className="px-1 md:p-3 py-2 flex md:flex-row gap-2 ">
              <p className=" text-sm sm:text-base leading-normal capitalize truncate">{item?.name ?? "-"}</p>
            </td>

            {/* Email */}
            <td className="px-2 py-1     hidden md:table-cell">
              <p className=" text-sm sm:text-base leading-normal truncate">{item?.email ?? "-"}</p>
            </td>

            {/* Phone */}
            <td className="px-2 py-1     hidden md:table-cell">
              <p className=" text-sm sm:text-base leading-normal capitalize truncate">{item?.mobile_number ?? "-"}</p>
            </td>

            {/* Role */}
            <td className="px-2 py-1     hidden md:table-cell">
              <button className="py-[4px] px-3 bg-primary-600 text-white group flex gap-1 items-center rounded-xl text-xs md:text-sm">
                {/* <MdRemoveRedEye className="text-white w-4 h-4" /> */}
                <p className=" hidden md:block">{item?.role_name ?? "-"}</p>
              </button>
            </td>
...
<div className="flex justify-center items-center my-10 relative gap-2">
  <button
    ...
    disabled={page === 1}
    className="px-2 py-2 mx-2 border rounded bg-primary-600 hover:bg-primary-700  disabled:opacity-50 disabled:cursor-not-allowed"
  >
    <HiChevronDoubleLeft className="w-6 h-auto text-white" />
  </button>
  ...
  >
    <HiChevronDoubleRight className="w-6 h-auto text-white" />
      ...
      /> */}
            {isFlyoutOpen && (
        <div
          className=" min-h-screen w-full  fixed top-0 left-0 right-0 z-[999]"
          onClick={() => {
      ...
      {({ values, handleChange, errors, touched }) => (
        <Form className="grid grid-cols-2 gap-6 mt-6">
          ...
          <div className="w-full relative mb-3">
            <p className="text-secondBlack text-base leading-normal mb-2">Your Name</p>
            <input
              type="text"
              name="name"
              value={values.name}
              ...
              className={`hover:shadow-hoverInputShadow focus-border-primary w-full h-[50px] border ${
                errors.name && touched.name ? 'border-red-500' : 'border-primary-100'
              } rounded-[4px] text-[15px] placeholder-gray-400 pl-4 mb-2 text-secondBlack `}
            />
          ...
          {/* Mobile Number */}
          <div className="w-full relative mb-3">
            <p className="text-secondBlack text-base leading-normal mb-2">Mobile Number</p>
            <input
              type="text"
              name="mobile_number"
              ...
              className={`hover:shadow-hoverInputShadow focus-border-primary w-full h-[50px] border ${
                errors.mobile_number && touched.mobile_number ? 'border-red-500' : 'borderprimary-100'
              } rounded-[4px] text-[15px] placeholder-gray-400 pl-4 mb-2 text-secondBlack`}
            />
          ...
          {/* Email */}
          <div className="w-full relative mb-3">
            <p className="text-secondBlack text-base leading-normal mb-2">Email</p>
            <input
              type="email"
              name="email"
              value={values.email}
              ...
              className={`hover:shadow-hoverInputShadow focus-border-primary w-full h-[50px] border ${
                errors.email && touched.email ? 'border-red-500' : 'border-primary-100'
              } rounded-[4px] text-[15px] placeholder-gray-400 pl-4 mb-2 text-secondBlack`}
            />
          ...
          {/* User Role (read-only) */}
          <div className="w-full relative mb-3">
            <p className="text-secondBlack text-base leading-normal mb-2">Role</p>
            <input
              type="text"
              value={values.role_name}
              readOnly
              className="w-full h-[50px] border border-primary-100 rounded-[4px] text-[15px] placeholder-gray-400 pl-4 mb-2 text-secondBlack outline-none cursor-not-allowed"
            />
